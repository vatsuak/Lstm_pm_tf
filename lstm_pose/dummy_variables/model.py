...
class Net():
    ...
    def lstm(self, heatmap, features, centermap, hide_t_1, cell_t_1):
        ...
        with tf.name_scope('LSTM'):
            ...
            gx = conv(xt,32+1+self.outclass,48,3)  # output: 45 * 45 * 48
            gh = conv(gx,48,48,3,bias=False)  # output: 45 * 45 * 48
            g_sum = tf.add(gx, gh)
            gt = tf.nn.tanh(g_sum)

            ox = conv(xt,32+1+self.outclass,48,3)  # output: 45 * 45 * 48
            oh = conv(hide_t_1,48,48,3,bias=False)  # output: 48 * 45 * 45
            o_sum = tf.add(ox, oh)
            ot = tf.nn.sigmoid(o_sum)

            ix = conv(xt,32+1+self.outclass,48,3)  # output: 45 * 45 * 48
            ih = conv(hide_t_1,48,48,3,bias=False)  # output: 48 * 45 * 45
            i_sum = tf.add(ix, ih)
            it = tf.nn.sigmoid(i_sum)

            fx = conv(xt,32+1+self.outclass,48,3)  # output: 48 * 45 * 45
